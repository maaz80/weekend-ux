...
export default function Details({ data }) {
     ...
     useEffect(() => {
          ...
          window.addEventListener("leadSubmitted", handleLeadSuccess);
          return () => {
               window.removeEventListener("leadSubmitted", handleLeadSuccess);
          };
     }, [data]);
                          ...
                          <div className="flex flex-wrap gap-4 justify-start">
                               {careerDomainsItems.map((item, idx) => (
                                    <a 
                                         key={idx} 
                                         href={item.link || "#"}
                                         className="flex items-center gap-3 bg-white px-6 py-4 rounded-2xl border border-zinc-200/80 hover:border-official hover:shadow-md hover:scale-[1.01] transition-all duration-300 group shrink-0"
                                    >
